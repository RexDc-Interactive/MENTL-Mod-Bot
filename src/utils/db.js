...
db.prepare(`
  CREATE TABLE IF NOT EXISTS oldusers (
    ...
    total_points INTEGER NOT NULL,
    warns TEXT,
    former_member INTEGER NOT NULL,
    PRIMARY KEY (user_id, guild_id)
  );
`).run();
...
const settings = {
  ...
  selectOwnerRoleId: db.prepare('SELECT owner_role_id FROM settings WHERE guild_id = ?;'),
  selectCoOwnerRoleId: db.prepare('SELECT co_owner_role_id FROM settings WHERE guild_id = ?;'),
  selectManagerRoleId: db.prepare('SELECT manager_role_id FROM settings WHERE guild_id = ?;'),
  ...
  updateOwnerRoleId: db.prepare('UPDATE settings SET owner_role_id = ? WHERE guild_id = ?;'),
  updateCoOwnerRoleId: db.prepare('UPDATE settings SET co_owner_role_id = ? WHERE guild_id = ?;'),
  updateManagerRoleId: db.prepare('UPDATE settings SET manager_role_id = ? WHERE guild_id = ?;'),
...
const oldusers = {
  insertRow: db.prepare(`
    INSERT OR IGNORE INTO oldusers (
      user_id, 
      user_name,
      user_discriminator,
      guild_id, 
...
module.exports = {
  ...
  oldusers,
	bannedusers
>>>>>>> 1fdb9d90d33ab8754b2fa39ea5e1dcecc68fc551
};