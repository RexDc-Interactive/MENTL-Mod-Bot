...
module.exports = class StatsCommand extends Command {
  constructor(client) {
    super(client, {
      ...
      usage: 'stats',
      description: 'Fetches MENTLS\'s statistics.',
      type: client.types.INFO
    });
  }
  async run(message) {
    const hd = moment.duration(await os.uptime());
    const hdays = (hd.days() == 1) ? `${hd.days()} day` : `${hd.days()} days`;
    const hhours = (hd.hours() == 1) ? `${hd.hours()} hour` : `${hd.hours()} hours`;
    ...
    const clientStats = stripIndent`
      ...
      Socket Status :: ${message.client.ws.status}
      Uptime    :: ${days} and ${hours}`;
    const { totalMemMb, usedMemMb } = await mem.info();
    const serverStats = stripIndent`
      OS          :: ${await os.oos()}
      Platform    :: ${await os.platform()}
      Host Uptime :: ${hdays} and ${hhours}
      CPU         :: ${cpu.model()}
      Cores       :: ${cpu.count()}
