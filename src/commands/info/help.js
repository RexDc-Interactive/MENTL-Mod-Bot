...
module.exports = class HelpCommand extends Command {
  ...
  run(message, args) {
    ...
    if (
      ...
      const total = Object.values(commands).reduce((a, b) => a + b.length, 0) - commands[BOTOWNER].length;
      const size = message.client.commands.size - commands[BOTOWNER].length;

      embed // Build help embed
        .setTitle('MENTL\'s Commands')
        .setThumbnail(message.client.user.avatarURL())
        .setDescription(stripIndent`
          **Prefix:** \`${prefix}\`
          ${(!all && size != total) ? `**Number of Commands:**\`${size}\`` : `**Total Number of Commands:**\`${total}\``}
