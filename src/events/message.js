    ...
    if (command && !disabledCommands.includes(command.name)) {
      ...
      if (modChannelIds.includes(message.channel.id)) {
        if (
          ...
          return; // Return early so MENTL doesn't respond
        }
      }

      // Check permissions
      const permission = command.checkPermissions(message);
      if (permission || isOwner(message.author)) {
      ...
      const embed = new MessageEmbed()
        .setTitle('Hi, I\'m MENTL. Need help?')
        ...
        .addField('Invite Me', oneLine`
          ...
          [here](https://discord.com/oauth2/authorize?client_id=789493502122786866&scope=bot&permissions=403008599)!
        `)
        .addField('Support', oneLine`
          If you have questions, suggestions, or found a bug, please join the 
          [MENTL Support Server](https://discord.gg/VPEgfjnzrV) (RexDc Interactive)!
        `)
        .setFooter('DM TheDireMaster#1987 to speak directly with the developer!')
